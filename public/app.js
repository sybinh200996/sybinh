...
function init(){renderTabs();renderHistory();loadAccount();loadVoicePrefs();initVietnameseVoices();startClock();const firstRoute=location.hash?.replace('#/','')||'home';routeTo(firstRoute,false);window.addEventListener('hashchange',()=>routeTo(location.hash.replace('#/','')||'home',false));checkGeminiStatus();loadAIProviders();setTimeout(()=>{if(window.matchMedia&&window.matchMedia('(max-width:760px)').matches&&$('chatText')){$('chatText').focus({preventScroll:true});}},600);}
...
async function quickAsk(){const q=(($('homeAsk')?.value||$('globalAsk')?.value||'')).trim();routeTo('chat');setTimeout(()=>{if(q){setChatTextValue(q);if($('homeAsk')) $('homeAsk').value='';sendChat()}else if($('chatText')){$('chatText').focus();}},120)}
function autoResizeHomeAsk(){const el=$('homeAsk');if(!el)return;el.style.height='auto';el.style.height=Math.min(el.scrollHeight,160)+'px';}
